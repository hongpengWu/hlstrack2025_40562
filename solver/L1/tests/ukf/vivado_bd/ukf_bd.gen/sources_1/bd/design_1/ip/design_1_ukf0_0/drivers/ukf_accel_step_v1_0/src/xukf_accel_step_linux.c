...
typedef struct {
    ...
    XUkf_accel_step_uio_map maps[ MAX_UIO_MAPS ];
} XUkf_accel_step_uio_info;

/***************** Variable Definitions **************************************/
static XUkf_accel_step_uio_info uio_info;
...
static int uio_info_read_name(XUkf_accel_step_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XUkf_accel_step_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XUkf_accel_step_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XUkf_accel_step_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XUkf_accel_step_Initialize(XUkf_accel_step *InstancePtr, const char* InstanceName) {
	XUkf_accel_step_uio_info *InfoPtr = &uio_info;

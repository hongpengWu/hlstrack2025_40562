...
SHA-256 (Secure Hash Algorithm 256-bit) 是一种密码散列函数，用于生成一个256位（32字节）的散列值，通常表示为64个十六进制数字。该算法的设计使其具有抗碰撞性，即找到两个不同的输入产生相同哈希值的计算上是不可行的。

SHA-256算法的主要步骤如下：

1. **消息填充 (Padding):** 对输入消息进行填充，以确保其长度是512位的倍数。填充过程包括：

   - 在消息末尾附加一个 '1' 位。
   - 附加 '0' 位，直到消息长度在模512下等于448。
   - 附加一个64位的整数，表示原始消息的长度（以位为单位）。
2. **初始化哈希值:** SHA-256使用8个32位的初始哈希值（H0-H7），这些值是前8个素数（2, 3, 5, 7, 11, 13, 17, 19）的小数部分的平方根的前32位。
3. **消息处理:** 填充后的消息被分成512位的块。每个块经过64轮计算。对于每一轮，都会进行以下操作：

   - **消息调度:** 将512位消息块扩展为64个32位字 (W0-W63)。
   - **压缩函数:** 使用扩展的消息字、8个工作变量（a-h）和64个常量（K0-K63）来更新工作变量。这些常量是前64个素数的立方根的小数部分的前32位。
...
$$
W_t = \sigma_1(W_{t-2}) + W_{t-7} + \sigma_0(W_{t-15}) + W_{t-16} \quad \text{for} \quad 16 \le t \le 63
$$
...
SHA-256的核心是迭代压缩函数，对每个512位消息块进行64轮运算：
...
- **`kpad` 模块**: 负责处理密钥 `K`，生成 `K' ⊕ ipad` 和 `K' ⊕ opad`。
  - `expandStrm`: 扩展密钥流。
  - `genPad`: 生成 `ipad` 和 `opad`。
  - `kpadHash`: 当密钥长度超过块大小时，对密钥进行哈希。
- **`msgHash` 模块**: 计算 `H((K' ⊕ ipad) || m)`。
  - `mergeKipad`: 将 `K' ⊕ ipad` 与消息 `m` 合并。
  - 内部调用 SHA-256 模块对合并后的数据进行哈希。
- **`resHash` 模块**: 计算 `H((K' ⊕ opad) || H((K' ⊕ ipad) || m))`。
  - `mergeKopad`: 将 `K' ⊕ opad` 与 `msgHash` 的结果合并。
  - 内部调用 SHA-256 模块对合并后的数据进行最终哈希，得到 HMAC 值。

##### 2.1.2.3 数据流图

```
密钥 (keyStrm) ───► kpad ───┬───► mergeKipad ───► msgHash ───┬───► mergeKopad ───► resHash ───► HMAC 值 (digestStrm)
                           │                                │
消息 (msgStrm)  ───────────┴───────────────────────────────►
```
...
该实现采用 AXI-Stream 接口（AP_FIFO）与顶层 `ap_ctrl_hs` 控制，适配 Zynq-7000 平台，易于与其他 IP 级联。

**接口规格（当前实现，hmac+sha256）：**

- 输入接口：
  - `keyStrm`: `hls::stream<ap_uint<32>>`，密钥数据（32-bit 宽度）。
  - `msgStrm`: `hls::stream<ap_uint<32>>`，消息数据（32-bit 宽度）。
  - `lenStrm`: `hls::stream<ap_uint<64>>`，消息长度（字节数）。
  - `eLenStrm`: `hls::stream<bool>`，长度流结束标志。
- 输出接口：
  - `hshStrm`: `hls::stream<ap_uint<256>>`，HMAC-SHA256 结果（256-bit）。
  - `eHshStrm`: `hls::stream<bool>`，结果流结束标志。
- 顶层控制与时钟复位：
  - `ap_clk`（clock）、`ap_rst`（reset）、`ap_ctrl_hs`（`ap_done/ap_idle/ap_ready/ap_start`）。

注：接口规格与 `test.cpp` 的 `test_hmac_sha256` 测试框架保持一致；`sha256_wrapper` 作为哈希包装器被 `hmac` 顶层模板调用。

##### 2.1.3.1 内部流与存储实现

- 主要内部流：`kipadStrm/kopadStrm/kopad2Strm/eKipadStrm/msgHashStrm/eMsgHashStrm/mergeKipadStrm/mergeKipadLenStrm/eMergeKipadLenStrm/mergeKopadStrm/mergeKopadLenStrm/eMergeKopadLenStrm`。
- 存储绑定：依据实现版本，内部 FIFO 绑定到 `LUTRAM` 或 `BRAM`，以平衡吞吐与资源。

#### 2.1.4 实现差异（当前 vs 基线）

- 顶层组织：两版均使用 `hmacDataflow` 将 `kpad → msgHash → resHash` 通过 `dataflow` 串接，两次调用 `sha256_top`（计算 `H(K'⊕ipad || m)` 与 `H(K'⊕opad || 前一结果)`）。
- 流深度与存储绑定差异：
  - 基线（报告中综合输出显示）：`mergeKipadStrm depth = 128 (BRAM)`；`eKipadStrm/kipadStrm/kopadStrm/kopad2Strm/msgHashStrm/eMsgHashStrm depth = 4 (LUTRAM)`；`mergeKopad{Len} depth = 4 (LUTRAM)`。
  - 当前版本（代码对比）：多数短通道深度提升至 8（SRL/LUTRAM），`mergeKipadStrm` 调整为 64（BRAM），`mergeKopad{Len}` 设为 2（BRAM/LUTRAM），以降低背压与提升数据流鲁棒性。
- SHA-256 内核差异：
  - 当前版本在 `sha224_256.hpp` 中对 `T1` 计算采用加法树与 `bind_op` 到 DSP 的表达式平衡，`generateMsgSchedule` 与 `sha256_iter` 使用更积极的 `pipeline/unroll/array_partition` 以降低关键路径与提高并行度。
  - 基线版本的流深度较小、消息调度与迭代实现更保守，关键路径较长。

#### 2.1.5 优化策略（SHA-256/HMAC）

- 数据流化：
  - 顶层 `hmacDataflow` 将 `kpad/msgHash/resHash` 并联/串接，两个 `sha256_top` 与周边合并模块形成稳定流水。
- 表达式与资源绑定：
  - `sha256_iter` 中 `T1 = h + Σ1(e) + Ch(e,f,g) + Kt + Wt` 的加法链采用分层加法树与 DSP 绑定，缩短关键路径。
- 调度与并行：
  - `generateMsgSchedule` 对 `W[16..63]` 通过 `array_partition` 与 `pipeline`（II=1）并行生成，避免数据相关形成瓶颈。
- FIFO 深度与层次：
  - 提高短通道深度至 8，`mergeKipadStrm` 选 BRAM 以支持长消息合并；`mergeKopad{Len}` 选用较浅深度减少资源。
- 其他：
  - `preProcessing/dup_strm` 的拷贝与分流保持 II=1；根据时序状态适当约束 `bind_storage`。

### 2.2 LZ4压缩算法原理

#### 2.2.1 算法原理

LZ4 可视为 LZ77 思路的轻量实现，强调极低的控制复杂度与高吞吐。压缩过程以“序列”为基本单位，每个序列以单字节 token 起始：高半字节给出字面量数量的基值，低半字节给出匹配长度的基值；当任一字段达到阈值 15 时，会通过后续的扩展字节对长度进行累加。随后依次写入字面量数据、两字节的回溯偏移（定位到历史窗口中的匹配起点）以及匹配长度的扩展值。

**序列组成（用更直白的表述）：**

- `Token`：1 字节，两段各 4 bit；高 4 bit 代表字面量基数，低 4 bit 代表匹配长度基数。
- `字面量扩展`：若 token 的字面量基数为 15，则追加一个或多个字节进行长度加和。
- `字面量数据`：直接拷贝的原始字节序列。
- `偏移量`：2 字节，指向此前未压缩数据中的匹配位置（窗口回溯）。
- `匹配长度扩展`：当匹配基数为 15 时，以追加字节的方式继续累加真实长度。

```
 ┌───────────┬───────────┬──────────────────┬───────────┬──────────────────┐
 │   Token   │ Literal   │  Literal Data    │  Offset   │  Match Length    │
 │ (1 byte)  │ Length    │ (Literal Length) │ (2 bytes) │ (Match Length)   │
 └───────────┴───────────┴──────────────────┴───────────┴──────────────────┘
```

**压缩效果度量：**

- 压缩比 = 原数据大小 / 压缩后数据大小，数值越大代表压缩越充分。

**匹配生成（窗口搜索思路）：**

- 在位置 `i` 的滑动窗口 `W=[i−MAX_OFFSET, i−1]` 中查找与 `S[i:]` 的最长公共前缀，得到 `(offset,length)`；`offset=i−j`，`length=LCP(S[j:],S[i:])`。

**实现步骤概述：**

- 快速匹配生成：哈希+字典滑窗定位候选匹配。
- 候选筛选：剔除次优，保留更长或更合算的匹配。
- 匹配增强：在本地窗口进一步延展匹配边界，提升压缩比。
- 序列封装：将字面量与匹配信息编码为符合 LZ4 规范的输出格式。

#### 2.2.2 系统架构设计

##### **2.2.2.1 顶层架构**:

顶层由 `lz4CompressEngineRun` 驱动一个轻量的流控外壳，输入/输出均为 AXI-Stream 端口。通过 `#pragma HLS dataflow` 将四段功能级流水前后级联，使它们在不同数据片段上并行推进；模块间以 `hls::stream` 解耦，测试侧在 `main` 完成文件装载与核调用。

```
Input ─▶ lzCompress ─▶ lzBestMatchFilter ─▶ lzBooster ─▶ lz4Compress ─▶ Output
```

##### **2.2.2.2 模块功能说明**

- **lzCompress**：基于滑窗与哈希字典生成候选匹配，同时标出字面量边界。
- **lzBestMatchFilter**：从候选集中剔除次优项，保留更长或更划算的匹配结果。
- **lzBooster**：利用局部缓存继续延展匹配边界，提升压缩比与序列质量。
- **lz4Compress**：按 LZ4 规范将字面量、偏移与匹配长度封装为输出字节流（内部含 Part1/Part2 状态机）。

##### 2.2.2.3 数据流图

```
Input Stream ─▶ lzCompress ─▶ lzBestMatchFilter ─▶ lzBooster ─▶ lz4Compress(Part1/Part2) ─▶ Output Stream
```

#### 2.2.3 接口设计

顶层调用 `lz4CompressEngineRun`，所有数据以 AXI-Stream 形态在模块间传递，避免握手与总线协议的额外开销。

**顶层端口（`lz4CompressEngineRun`）**

- 输入：
  - `inStream`（`hls::stream<ap_uint<8>>`）输入字节流
  - `input_size`（`uint32_t`）待压缩的总字节数
  - `max_lit_limit`（`uint32_t[1]`）字面量上限，单并行块配置
  - `core_idx`（`uint32_t`）核索引，便于多核场景定位
- 输出：
  - `lz4Out`（`hls::stream<ap_uint<8>>`）压缩后字节流
  - `lz4Out_eos`（`hls::stream<bool>`）输出结束标记
  - `lz4OutSize`（`hls::stream<uint32_t>`）压缩块大小统计

**内部约束与参数**

- 通道深度：`compressdStream`/`bestMatchStream`/`boosterStream` 统一设为 `depth=8`。
- 存储绑定：`compressdStream` 与 `boosterStream` 指定 `BIND_STORAGE FIFO=SRL`，以寄存器移位实现轻量化缓冲。
- 调度与配置：顶层采用 `dataflow` 并行推进；关键参数固定为 `MATCH_LEN=6`、`c_minMatch=4`、`LZ_MAX_OFFSET_LIMIT=65536`、`MAX_LIT_COUNT=4096`、`MAX_MATCH_LEN=255`。
...
### 3.2 SHA-256/HMAC 优化策略设计

#### 3.2.1 瓶颈分析（结合当前报告）

我们对当前 HMAC-SHA256 测试工程（`test_hmac_sha256`）进行分析，参考 `csynth.rpt` 与 `test_hmac_sha256_cosim.rpt`：

* **sha256Digest函数**: 单块 64 轮迭代，报告 `68` 周期（含状态更新，瓶颈路径约 `851.292 ns`）。
...
3. **存储访问与流背压**：K 常量与 W 调度的读写在不同实现中可能落在 LUTRAM/BRAM；同时 HMAC 合并阶段（`mergeKipad/mergeKopad`）的 FIFO 深度影响背压与吞吐。
...
我们通过 `bind_storage`/`stream depth` 配置将：
- `K` 常量与短通道 FIFO 优先映射到 `LUTRAM/SRL`，降低访问延迟；
- 长通道（如 `mergeKipadStrm`）映射到 `BRAM`，维持更大深度支持长消息拼接；
- 将多个短通道深度由 4 提升至 8，以减少偶发背压对 dataflow 的影响。
...
* `generateMsgSchedule` 生成 64 个 `W` 并输出到流；
* `sha256Digest` 从 `W` 流读取并执行 64 轮压缩（II=1），两者在顶层通过 `dataflow` 并行调度。
...
围绕 LZ4 的实现，我们以“更快的流推进、更稳的时序、更少的停顿”为方向，明确三类目标：

- 提升流水性能：压缩内核与封装模块的关键循环尽量保持 `II=1`，缩短单位工作量的周期开销；
- 收敛总延迟：输入到输出的端到端时间压缩，减少背压引发的停顿；
- 平衡存储访问：缓解哈希字典与本地缓冲的端口冲突，稳定数据供给。

#### 3.3.1 存储优化

思路：哈希/字典结构是匹配生成的“供给侧”，端口不够与访存冲突是典型瓶颈。通过结构拆分与端口扩张，降低因读写竞争导致的流水停顿。

措施：

- 数组分割（`ARRAY_PARTITION`）：针对核心哈希表按维度完全分割，形成多片并发访存单元，减轻热点冲突，支撑 `II=1` 的循环调度。
...
原理：将循环迭代在时间上交叠，使每拍均可接纳新数据，维持稳态吞吐。

措施：

- 关键循环 `PIPELINE(II=1)`：在压缩、筛选、增强与封装的主循环中维持 `II=1`，降低串行等待；必要时拆解组合逻辑与控制路径以缓解时序压力。

#### 3.3.3 数据流并行优化

原理：把多阶段串行工作改造成生产者-消费者流水，各阶段独立推进，通过通道解耦时序依赖。

措施：

1. 模块化：保持 `lzCompress`/`lzBestMatchFilter`/`lzBooster`/`lz4Compress` 四段分工明确；
2. 通道通信：以 `hls::stream` 连接各段，避免强耦合，提升并行度与稳态流量。

```cpp
// 数据流并行示意
#pragma HLS DATAFLOW
// ... pipeline modules connected via hls::stream ...
```
...
##### 指标总览
- 平台与配置：`xc7z020-clg484-1`，`DIM=3`，`LOWER_TRIANGULAR=0`，顶层 `kernel_cholesky_0`。
- HLS 综合（`csynth.rpt`）：延迟 `1038` cycles（≈ `5.380e+03 ns`），时钟周期 `5.18 ns`，Timing Slack `-2.75 ns`。
- 资源利用（`csynth.rpt`）：`DSP 14 (6%)`，`FF 17142 (16%)`，`LUT 12608 (23%)`。BRAM/URAM 未使用。
- RTL 仿真（`kernel_cholesky_0_cosim.rpt`）：功能通过；计算延迟 `876` cycles；总执行时间 `7015` cycles。
- C 仿真（`kernel_cholesky_0_csim.log`）：3×3用例均通过（`TB:Pass`，0 errors），包含 `L Matching / DUT Ratio / LAPACK Ratio / Relative Ratio Difference` 明细。

##### 架构选择与实现映射
- 架构切换逻辑：`choleskyTop` 根据 traits 的 `ARCH` 选择实现：`0→choleskyBasic`，`1→choleskyAlt`，`2→choleskyAlt2`。
- 当前调用：`kernel_cholesky_0.cpp` 未将 `my_cholesky_traits` 作为模板参数传入 `xf::solver::cholesky(...)`，因此使用默认 traits（`ARCH=1`）。即使 `SEL_ARCH=0` 在编译参数中定义，因未传入 traits，宏未生效。
- 证据：合成生成的 RTL 名称含 `choleskyAlt`；默认 `choleskyTraits` 定义 `ARCH=1`（路径 `solver/L1/include/hw/cholesky.hpp`）。
- 切换建议：如需使用 `ARCH=0`，调用 `choleskyTop<LOWER_TRIANGULAR, DIM, my_cholesky_traits, MATRIX_IN_T, MATRIX_OUT_T>(...)` 或调整默认 traits 的 `ARCH`，并重新评估时序与资源。

##### 指标解读与瓶颈
- 时序：Slack 为负（`-2.75 ns`），表明当前约束下未收敛；需在关键环路（`row_loop / col_loop / sum_loop`）进一步减小关键路径或降低并行度。
- 延迟差异：综合延迟 `1038` cycles 高于 RTL 仿真 `876` cycles，属估算与仿真环境差异范围。
- 资源：`DSP=14` 保持不变；`LUT=23%` 与 `FF=16%` 在 Zynq-7020 可接受区间；BRAM/URAM 未使用。

##### 后续优化方向（计划）
- 保持接口不变、以 `ARCH=1` 为基线的增量优化：
  - 在 `sum_loop` 保持 `II=1`，结合适度 `UNROLL` 与 `ARRAY_PARTITION` 提升带宽。
  - 对 `L_internal / diag_internal` 采用更细粒度分区以缓和路由拥塞。
  - 预计算索引以减少乘除法开销，缩短关键路径。
  - 视时序结果动态调节展开因子，换取正 Slack。
- 若切换到 `ARCH=0`，需对一维索引与访存路径单独评估其时序影响。

##### 数据来源
- `solver/L1/tests/cholesky/complex_fixed_arch0/reports/csynth.rpt`
- `solver/L1/tests/cholesky/complex_fixed_arch0/reports/kernel_cholesky_0_cosim.rpt`
- `solver/L1/tests/cholesky/complex_fixed_arch0/reports/kernel_cholesky_0_csim.log`
...
数据来源：`test/sha256_test/reports/csynth.rpt`“Performance & Resource Estimates”顶层行（`test_hmac_sha256`）。
...
为保证客观性，本小节以当前测试工程 `test_hmac_sha256` 的综合与联合仿真报告为准：

- 报告来源：
  - 当前测试工程：`test/sha256_test/reports/csynth.rpt` 与 `test/sha256_test/reports/test_hmac_sha256_cosim.rpt`
  - 优化版本（v 路径）：`v/hlstrack2025/security/L1/tests/hmac/sha256/reports/summary_T_exec.txt`
- 顶层：`test_hmac_sha256`（包含 `hmacDataflow` 与两次 `sha256_top`）

| 性能指标                             | 当前值（test 路径）         | 优化值（v 路径）  | 备注 |
| ------------------------------------ | --------------------------- | ----------------- | ---- |
| 估计时钟周期(Estimated_Clock_Period) | ≈ 12.52 ns                  | 7.565 ns          | test 由 digest 段推算；v 为汇总文件给出 |
| 延迟(Cosim_Latency_周期)             | 690                         | 798               | test 为 cosim 报告；v 为汇总文件最大值 |
| 执行时间(T_execution)                | ≈ 8,638.8 ns                | 6109.488 ns       | `12.52 ns × 690` vs 汇总文件计算 |
| 目标时钟周期(Target_Clock_Period)    | —                           | 8.6 ns            | v 汇总文件给出 |
| 时序状态（Slack）                    | -10.24 ns（最差路径）       | —                 | test 综合表顶层行 |

注：`test` 路径综合摘要未直接显示全局 `Estimated Clock Period`，此处以瓶颈段近似值代替；`v` 路径提供了更完整的汇总（`summary_T_exec.txt`）。两者用于分别记录当前测试工程与优化版本的客观指标。

#### 5.2.3 LZ4 资源使用对比

| 资源类型 | 优化前 | 优化后 | 改善幅度 | 利用率(优化前) | 利用率(优化后) |
| -------- | ------ | ------ | -------- | -------------- | -------------- |
| BRAM_18K | 106    | 192    | +81.1%   | 37.9%          | 68.6%          |
| DSP      | 0      | 0      | 0%       | 0%             | 0%             |
| LUT      | 7,231  | 11,504 | +59.1%   | 13.6%          | 21.6%          |
| FF       | 3,537  | 3,760  | +6.3%    | 3.3%           | 3.5%           |
...
| 时钟频率(Clock_Frequency)            | 66.67 MHz   | 71.9 MHz        | +7.8%    |
| 延迟(Cosim_Latency_周期)             | 3,390       | 1,599           | -52.8%   |
| 执行时间(T_execution)                | 44,815.8 ns | 19,923.5 ns     | -55.6%   |
